...
from backend.v8.v85_variants import run_v85_variant_builder
...
def run_v8_pipeline(
    ...
    try:
        print("[V7.5] loading registries")
        load_style_registries()
        print("[V7.5] registries loaded")
        print("[V7.5] building style policy")
        policy = build_style_policy(topic=topic, keyword=topic, audience=audience or None)
        print(f"[V7.5] selected channel_style_id={policy['channel_style_id']}")
        print(f"[V7.5] selected image_style_id={policy['image_style_id']}")
        print(f"[V7.5] selected thumbnail_style_id={policy['thumbnail_style_id']}")
        print(f"[V7.5] selected prompt_system_id={policy['prompt_system_id']}")
        print(f"[V7.5] policy_fingerprint={policy['policy_fingerprint']}")
        style_policy_path = v8_root / "style_policy.json"
        save_style_policy(policy, str(style_policy_path))
        print(f"[V7.5] style policy saved path={style_policy_path}")
    ...
    run_v85_variant_builder(v8_root=v8_root, run_id=run_id)
