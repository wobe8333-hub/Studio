...
hooks:
  ...
  # SubagentStop 전역 훅이 pytest·ruff·npm build 품질 게이트를 단일 책임으로 담당
initialPrompt: |
...
## 자가 치유
테스트 실패 시 최대 3회 자동 수정 → 실패 시 cto에게 에스컬레이션.

## API 변경 시
web/app/api/ 계약 변경 시 frontend-engineer에게 SendMessage 사전 알림 필수.

## Reflection 패턴 (세션 종료 전)

미션 완료 후 `~/.claude/agent-memory/backend-engineer/MEMORY.md` 에 기록:
