...
- 앱 로직 내 직접 쿼리 수정 금지 — python-dev/web-dev에 SendMessage 위임

## 의사결정 3종 세트
1. SQL 마이그레이션 + RLS 정책 + types.ts 동기화 (3개 동시)
2. RLS 1차 설계 담당 — quality-security는 감사만
3. 파괴적 변경(DROP, 타입 축소) 시 백필 스크립트 필수

## 에스컬레이션
- 다운타임 위험 → mission-controller
- 앱 코드 동시 변경 → python-dev + web-dev 병렬 소환 요청
