...
- **SessionStart 자동 감지**: `settings.local.json`의 `SessionStart` 훅이 매 세션 시작 시 `scripts/mission_probe.py`를 비차단 실행 — HITL 미해결 신호·FAILED 런 감지 후 `/mission` 권고. 세션 간 자율 기동의 세션 내 대안.
