...
"""
        ...
        for line in text.split("\n"):
            ...
            if line.startswith("VERIFIED:"):
                ...
                fact = parts[1] if len(parts) > 1 and parts[0].isdigit() else content
                if fact:
    ...
    if len(pkg.core_facts) < 3:
        ...
        try:
            ...
            resp = model.generate_content(
                f"{pkg.topic}에 대한 핵심 사실 5가지를 한국어로 간결하게 나열해주세요."
            )
            lines = [l.strip().lstrip("0123456789.-• ") for l in resp.text.split("\n")
                     if len(l.strip()) > 15]
