"""배너 재개 — 기존 파일 건너뛰고 미완성 variant만 생성 (variant_1~10 × 7채널).

사용법:
    python scripts/generate_branding/gen_assets_banner_resume.py
"""
import sys, io
...
def main():
    client = _make_client()
    print("=== 배너 재개 생성 (variant_1~10, 기존 파일 스킵) ===\n")
    ...
    for ch_id, theme in CHANNEL_THEMES.items():
        ...
        if not missing:
            print(f"[{ch_id}] 이미 완료 — 스킵\n")
            continue

        print(f"[{ch_id}] 미완성 {len(missing)}장 생성 중: {missing}")
        saved = 0
        for i in missing:
        ...
        print(f"[{ch_id}] 완료: {saved}/{len(missing)}장\n")
